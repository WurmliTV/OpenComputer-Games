...
function spawn_fruit()
    local old_color = gpu.setForeground(0xCC0000)
...
function update_player()
    ...
    -- Add new Head
    term.setCursor(player[1].x, player[2].y)
    print("o")
...
-- Handle dead screen
function dead_screen(address, char, code, playerName)
    return code == 13
end
...
local reset_color = gpu.setForeground(0x05b435)

while running do
    game_loop()
    os.sleep(tick_time) -- To prevent lags
...
if ded == true then
    ...
    while dead_screen(event.pull("key_down")) ~= true do end
end

-- CLeanup the screen and set initial values
